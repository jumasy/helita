...
class Mah:
    ...
    def read_dat4(self):
        ...
        for it in range(self.input_ini['nt1sav']):
            try:
                for idx, iv in enumerate(varlist):
                    self.input_dat4[iv][:, :, it] = (np.fromfile(f, dtype='float32', count=np.size(self.input_dat4[iv][..., 0]))).reshape((
    ...
    def read_dat2(self):
        ...
        for it in range(self.input_ini['nt2sav']):
            try:
                for idx, iv in enumerate(varlista):
                    self.input_dat2[iv][:, :, it] = (np.fromfile(f, dtype='float32', count=np.size(self.input_dat2[iv][..., 0])))*reshape((
    ...
    def read_dat3(self):
        ...
        for it in range(self.input_ini['nt2sav']):
            try:
                for idx, iv in enumerate(varlist):
                    self.input_dat3[iv][:, :, it] = (np.fromfile(f, dtype='float32', count=np.size(self.input_dat3[iv][..., 0]))).reshape((
    ...
    def read_dat5(self):
        ...
        for it in range(self.input_ini['nt5sav']):
            # try:
            for idx, iv in enumerate(varlista):
                self.input_dat5[iv][..., it] = (np.fromfile(f, dtype='float32', count=np.size(self.input_dat5[iv][..., 0]))).reshape((
    ...
    def read_dat6(self):
        ...
        for it in range(self.input_ini['nt6sav']):
            try:
                for idx, iv in enumerate(varlista):
                    self.input_dat6[iv][:, :, it] = (np.fromfile(f, dtype='float32', count=np.size(self.input_dat6[iv][..., 0])))*reshape((
